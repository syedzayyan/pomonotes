document.addEventListener("DOMContentLoaded", () => {
  ...
  function loadSessions() {
    ...
    fetch("/api/sessions")
        ...
        if (sessions && sessions.length > 0) {
          ...
          sessions.forEach((session) => {
            ...
            if (session.tags && session.tags.trim() !== "") {
              const tagList = session.tags.split(",").map((tag) => tag.trim());
              tagsHTML = tagList
                .map((tag) => `<span class="session-tag">${tag}</span>`)
                .join("");
            }
            ...
            row.innerHTML = `
                        ...
                        <td class="session-tags-cell">${tagsHTML}</td>
                        <td>
      ...
      .catch((error) => {
        ...
        sessionsTableBody.innerHTML =
          '<tr><td colspan="7">Error loading sessions. Please try again.</td></tr>';
      });
  }
  ...
  function setupSessionClickHandlers() {
    ...
    document.querySelectorAll(".delete-session").forEach((button) => {
      button.addEventListener("click", function (event) {
        ...
        if (
          confirm(
            "Are you sure you want to delete this session and all its notes?",
          )
        ) {
  ...
  function openSessionDetails(sessionId) {
    ...
    fetch(`/api/sessions/${sessionId}`)
      ...
      .then((session) => {
        ...
        let tagsHTML = '<span class="no-tags">No tags</span>';
        if (session.tags && session.tags.trim() !== "") {
          const tagList = session.tags.split(",").map((tag) => tag.trim());
          tagsHTML = tagList
            .map((tag) => `<span class="session-tag">${tag}</span>`)
        ...
        sessionDetails.innerHTML = `
                <dl>
                    ...
                    <dt>Tags</dt>
                    <dd class="session-tags">${tagsHTML}</dd>
                </dl>
            `;
    ...
    document.querySelectorAll(".edit-note").forEach((button) => {
      button.addEventListener("click", function () {
        ...
        const markdownContent =
          noteItem.querySelector(".note-content").dataset.markdown ||
        ...
        noteItem
          ...
          .addEventListener("click", function () {
            const updatedNote = editNoteEditor
              ? editNoteEditor.value()
  ...
  function addNote() {
    const noteText = addNoteEditor
      ? addNoteEditor.value()
      : addNoteTextarea.value.trim();
  ...
  document.addEventListener("click", (event) => {
    ...
    if (
      event.target.id === "editor-cancel-btn" ||
      event.target.id === "editor-close"
    ) {
      closeFullPageEditor();
    }
  });
