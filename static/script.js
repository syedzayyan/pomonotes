document.addEventListener('DOMContentLoaded', () => {
    ...
    function executeStopTimer() {
        ...
        // Update session in database with current progress
    ...
    function updateBreakStatus(status) {
        ...
        const now = new Date().toISOString();
        const breakLength = currentPomodoro % 4 === 0 ? longBreakLength : shortBreakLength;
    ...
    function updateSessionProgress() {
        ...
        updateSessionStatus(completedPomodoros >= 4 ? "completed" : "in-progress", totalTime, completedPomodoros);
    }
    ...
    function saveNote() {
        const noteText = markdownEditor.value.trim();
        if (!noteText) {
        ...
        .then(data => {
            ...
            markdownEditor.value = ''; // Clear the editor
            markdownPreview.innerHTML = ''; // Clear the preview
        })
        .catch(error => console.error('Error saving note:', error));
    }

    // Markdown editor preview functionality
    if (markdownEditor) {
    ...
    if (listBtn) {
        listBtn.addEventListener('click', () => {
            const selection = markdownEditor.value.substring(
    ...
    function insertMarkdown(prefix, suffix, placeholder) {
        ...
        const selection = markdownEditor.value.substring(start, end);
        ...
        if (!selection) {
            ...
            markdownEditor.selectionEnd = start + prefix.length + placeholder.length;
        } else {
            markdownEditor.selectionStart = start + prefix.length + selection.length + suffix.length;
        }
    }
    
    // Helper for replacing selection
    ...
    function initWeeklyChart() {
        fetch('/api/sessions')
            ...
            .catch(error => console.error('Error loading sessions for chart:', error));
    }
